...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let all_transactions = client.list_all_transactions_history(
        merchant_code,
        Some("desc"),
        Some(5), // Limit to 5 pages
    ).await?;
    
    println!("  Fetched {} transactions across all pages", all_transactions.len());
    ...
    match client.list_checkouts_with_query(&query).await {
        Ok(checkouts) => {
            println!("  Found {} paid checkouts with reference 'order-123'", checkouts.len());
