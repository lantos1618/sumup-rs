use sumup_rs::{SumUpClient, CreateCustomerRequest, UpdateCustomerRequest};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.retrieve_customer(&customer.customer_id).await {
        Ok(retrieved_customer) => {
            ...
            println!("   Name: {} {}", 
                retrieved_customer.personal_details.as_ref()
    ...
    match client.update_customer(&customer.customer_id, &update_request).await {
        Ok(updated_customer) => {
            println!("✅ Customer updated successfully!");
            println!("   Updated Name: {} {}", 
                updated_customer.personal_details.as_ref()
                    .and_then(|pd| pd.first_name.as_ref())
                    .unwrap_or(&"N/A".to_string()),
                updated_customer.personal_details.as_ref()
                    .and_then(|pd| pd.last_name.as_ref())
                    .unwrap_or(&"N/A".to_string()));
            println!("   Updated Email: {}", updated_customer.personal_details.as_ref()
                .and_then(|pd| pd.email.as_ref())
                .unwrap_or(&"N/A".to_string()));
        }
        Err(e) => {
    ...
    match client.list_customer_payment_instruments(&customer.customer_id).await {
        Ok(instruments) => {
